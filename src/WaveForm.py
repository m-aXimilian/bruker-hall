...
class WaveForm():
    ...
    def __init__(self, amp, n, zero = 0) -> None:
        ...
        self.N = n
        if zero > 1 or zero < 0:
            raise ValueError("Zero periode  in {} cannot exeed 100% or be negative. (zero = {}%)"
                             .format(self.__class__.__name__, zero * 100))
    ...
    def triangle(self):
        ...
        saw = (signal.sawtooth(2*np.pi*np.linspace(0, 1, ramp), 0.5) + 1) / 2 * self._amp
