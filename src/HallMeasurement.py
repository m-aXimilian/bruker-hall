...
class HallMeasurement:
    ...
    def __init__(self, config_file=""):
        ...
        self.lockin = sr830.Lockin(
            self.params["devices"]["lockin"]["id"], self.measure["settings"]
        )
    ...
    def read_field(self):
        ...
        return (
            self.readVolt(self.tasks["reader"])[self.measure_index]
            / self.lookup["pid-scale"]
        )
    ...
    def __generateTasks(self):
        ...
        self.tasks["reader"].add_channels(
            ...
            self.params["devices"]["daq-card"]["ai"],
            _type="ai",
        )
        tmp = None
        ...
        for k, v in self.params["devices"]["daq-card"]["ao"].items():
            ...
            self.tasks[tmp_name].add_channels(
                self.params["devices"]["daq-card"]["id"], v, _type="ao"
            )
    ...
    def __generateWave(self):
        ...
        if check > self.measure["settings"]["max-inc"]:
            e = "Field increment with {}mT higher than the allowed {}mT".format(
                check, self.measure["settings"]["max-inc"]
            )
            logging.error(e)
            raise TypeError(e)

        logging.info(
            "H-field vector ok. Dim: {}, increment: {}".format(tmp.shape, check)
        )
        self.set_field = tmp
    ...
    def single_xanterx_set(self, v, d) -> float:
        ...
        if d == DIRECTION.UP or d == DIRECTION.NONE:
            return (
                self.xantrex_lookup["rampup"].getValue(v)
                / self.params["devices"]["xantrex"]["range"]["high"]
                * 10
            )

        return (
            self.xantrex_lookup["rampdown"].getValue(v)
            / self.params["devices"]["xantrex"]["range"]["high"]
            * 10
        )
