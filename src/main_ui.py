...
test_data = np.genfromtxt(
    "tests/results/2022-02-09_15-50-08bruker-time-constant_to599mT.csv",
    delimiter=",",
    skip_header=1,
    names=True,
)
...
class MainWidget(QWidget):
    ...
    def generate_config_dict(self):
        self.conf = {
            "wave": self.__dict_convert(self.wave),
    ...
    def load_conf_button_handler(self):
        f = QFileDialog.getOpenFileName(
            ...
            filter="Configuration Files (*.yaml *.yml)",
        )
    ...
    def wave_conf(self):
        ...
        for k, v in self.wave.items():
            ...
            if isinstance(v, QLineEdit):
                v.textChanged.connect(
                    (lambda: self.enable_button(self.save_conf_button))
                )
            if isinstance(v, (QSpinBox, QDoubleSpinBox)):
